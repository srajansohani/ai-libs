...
train_data_input = torch.from_numpy(train_data_input.to_numpy())
train_data_expected_output = torch.from_numpy(train_data_expected_output.to_numpy().reshape(-1, 1))



test_data_input = torch.from_numpy(test_data_input.to_numpy())
test_data_expected_output = torch.from_numpy(test_data_expected_output.to_numpy().reshape(-1,1))
...
model.compile(batch_size=32,epochs=100,learning_rate=0.01,loss=Loss.MSE)

train_data_input = train_data_input.float()
